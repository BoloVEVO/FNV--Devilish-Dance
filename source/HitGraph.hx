...
class HitGraph extends Sprite
{
	...
	private function drawHitNotes()
	{
		...
		{
			var x_position = (history[i].strumTime / #if cpp (PlayState.instStream.length / PlayState.instance.songMultiplier) #else (PlayState.instance.inst.length / PlayState.instance.songMultiplier) #end) * (_width);
